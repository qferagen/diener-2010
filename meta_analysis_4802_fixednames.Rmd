...
```{r}
library(readxl)
big_data <- read_excel("C:/Users/kreig/Downloads/meta_analysisDat (4).xlsx")
...
```{r}
full_data_names <- rbind.data.frame(hongKong, vienna, canada, america)
...
wellbeingBasicNeedsModel <- lm(ladder ~ BasicNeeds , data = dat)
wellbeingPsychNeedsModel <- lm(ladder ~ BasicNeeds + PsychNeeds , data = dat)
wellbeingLuxuryModel<- lm(ladder ~ BasicNeeds + PsychNeeds + LuxuryConveniences + livingStandard , data = dat)
wellbeingIncomeModel <- lm(ladder ~ BasicNeeds + PsychNeeds + LuxuryConveniences + livingStandard + logIncome , data = dat)
...
```{r}
view(dat)
positiveBasicNeedsModel <- lm(PositiveFeelings ~ BasicNeeds , data = dat)
summary(positiveBasicNeedsModel)

positivePsychNeedsModel <- lm(PositiveFeelings ~ BasicNeeds + PsychNeeds, data = dat)
summary(positivePsychNeedsModel)

positiveLuxuryModel <- lm(PositiveFeelings ~ BasicNeeds + PsychNeeds + LuxuryConveniences + livingStandard , data = dat)
summary(positiveLuxuryModel)

positiveIncomeModel <-lm(PositiveFeelings ~ BasicNeeds + PsychNeeds + LuxuryConveniences + livingStandard + logIncome , data = dat)
summary(positiveIncomeModel)
``` 
 
```{r} 
negativeBasicNeedsModel <- lm(NegativeFeelings ~ BasicNeeds , data = dat)
summary(negativeBasicNeedsModel)

negativePsychNeedsModel <- lm(NegativeFeelings ~ BasicNeeds + PsychNeeds , data = dat)
summary(negativePsychNeedsModel)

negativeLuxuryModel <- lm(NegativeFeelings ~ BasicNeeds + PsychNeeds + LuxuryConveniences + livingStandard , data = dat)
summary(negativeLuxuryModel)

 negativeIncomeModel <-lm(NegativeFeelings ~ BasicNeeds + PsychNeeds + LuxuryConveniences + livingStandard + logIncome , data = dat)
summary(negativeIncomeModel)


wellbeingAnova <- anova(wellbeingBasicNeedsModel, wellbeingPsychNeedsModel, wellbeingLuxuryModel, wellbeingIncomeModel)
wellbeingAnova
positiveAnova <-  anova(positiveBasicNeedsModel, positivePsychNeedsModel, positiveLuxuryModel, positiveIncomeModel)
positiveAnova
negativeAnova <- anova(negativeBasicNeedsModel, negativePsychNeedsModel, negativeLuxuryModel, negativeIncomeModel)
negativeAnova
...
#graph of Income and Positive Feelingshttp://127.0.0.1:12299/graphics/45989624-bb1f-4f32-bb9d-e3513c96ab48.png
